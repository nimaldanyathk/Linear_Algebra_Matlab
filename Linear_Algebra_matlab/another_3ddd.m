

%very important question for 3d span
clc;
clear all;
close all;

v1 = [1;2]; % First vector
v2 = [2;1]; % Second vector

[a, b] = meshgrid(-2:1:2, -2:1:2); % Generate integer combinations from -2 to 2

X = a * v1(1) + b * v2(1); % Compute x-coordinates
Y = a * v1(2) + b * v2(2); % Compute y-coordinates

figure;
hold on;
grid on;

scatter(X(:), Y(:), 50, 'b', 'filled'); % Plot the span of vectors

% Plot vectors
quiver(0, 0, v1(1), v1(2), 0, 'r', 'LineWidth', 2, 'MaxHeadSize', 0.5);
quiver(0, 0, v2(1), v2(2), 0, 'g', 'LineWidth', 2, 'MaxHeadSize', 0.5);

% Labels and Titles
xlabel('X-axis');
ylabel('Y-axis');
title('Span of Two Vectors in 2D');
legend('Span Points', 'Vector 1', 'Vector 2', 'Location', 'best');
axis equal;